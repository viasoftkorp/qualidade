...
SELECT
    ...
    COALESCE(PPEDLISE_ORDEM_FABRICACAO.NUMODF, ESTOQUE_LOCAL.ODF) AS OdfApontada,
	...
	EMPRESA_RECNO = COALESCE(PPEDLISE.EMPRESA_RECNO, FECHA.EMPRESA_RECNO)), 0)
	...
	ISNULL((
		...
		AND QA_INSPECAO_SAIDA.NUMODF = ESTOQUE_LOCAL.ODF
...
const GetEstoqueLocalRelatorio = `
SELECT
	DISTINCT
	ESTOQUE_LOCAL.CODIGO AS CodigoProduto,
	ESTOQUE_LOCAL.LOTE AS Lote,
	...
	ESTOQUE.DESCRI AS DescricaoProduto
FROM ESTOQUE_LOCAL
INNER JOIN ESTOQUE ON ESTOQUE.CODIGO = ESTOQUE_LOCAL.CODIGO
WHERE
ESTOQUE_LOCAL.EMPRESA_RECNO = @` + NamedEmpresaRecno + `
AND ESTOQUE_LOCAL.ODF = @` + NamedOdf + `
`
