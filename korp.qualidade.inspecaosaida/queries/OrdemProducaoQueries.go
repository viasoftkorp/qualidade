...
CTE_ENTRADA_CQ_SAIDA_NAO_ESTORNADOS AS (
	...
	FROM HISREAL
	WHERE LOCAL_DESTINO IN (SELECT CODIGO FROM LOCAIS WHERE LOCAL_CQ_SAIDA = 'S') AND EMPRESA_RECNO = @` + NamedEmpresaRecno + "\n" + ` AND ESTORNADO_APT_PRODUCAO = 'N' AND ((MOV_EFETIVA = 'S' AND FORMA = 'E') OR (MOV_EFETIVA = 'N' AND FORMA = 'P' AND OPERACAO IS NOT NULL)) -- Esse OR é para o cenario de ODF de retrabalho
...
SELECT TOP 1 Cliente.RASSOC FROM PCP_ODF_PEDIDO AS OdfPedido
...
AND Cliente.RASSOC IS NOT NULL
`
