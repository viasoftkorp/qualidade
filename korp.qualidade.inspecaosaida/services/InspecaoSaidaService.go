...
func (service *InspecaoSaidaService) ImprimirInspecaoSaida(codigoInspecao int) ([]byte, *dto.ValidacaoDTO) {
	inspecao, err := service.InspecaoSaidaRepository.BuscarInspecaoSaidaPeloCodigo(codigoInspecao)
	if err != nil {
		return nil, &dto.ValidacaoDTO{
			Message: err.Error(),
	...
	ordem := service.OrdemProducaoRepository.BuscarEstoqueLocalRelatorio(inspecao.Odf)
	...
	dataEmissao := time.Now().Format("2006-01-02 15:04:05")
