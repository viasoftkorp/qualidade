...
func BuscarNotasRelatorio(ctx *fiber.Ctx) error {
	codInspecao, err := strconv.Atoi(ctx.Params("codInspecao", ""))
	if err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": err.Error()})
	}
	...
	notas, err := inspecaoSaidaService.BuscarNotasRelatorio(codInspecao, baseFilters)

	if err != nil {
		return ctx.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	} else {
		return ctx.Status(fiber.StatusOK).JSON(notas)
	}
}
...
func ImprimirInspecaoSaida(ctx *fiber.Ctx) error {
	...
	err = ctx.QueryParser(&notaFiscal)
	if err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": err.Error()})
	}
	...
	bytes, validacaoDTO := inspecaoSaidaService.ImprimirInspecaoSaida(codInspecao, notaFiscal)

	if validacaoDTO != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(validacaoDTO)
	} else {
		return ctx.Status(fiber.StatusOK).JSON(bytes)
	}
}
