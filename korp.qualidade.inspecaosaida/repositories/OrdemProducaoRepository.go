...
func (repo *OrdemProducaoRepository) AplicarFiltros(query string, baseFilters *models.BaseFilter, filters *dto.OrdemProducaoFilters) string {
	...
	if len(filters.ObservacoesMetricas) > 0 {
		...
		joinFiltersQuery +=
			"JOIN QA_INSPECAO_SAIDA ON QA_INSPECAO_SAIDA.NUMODF = COALESCE(HISREAL.ODF, CTE_SALDO_CQ.ODF)\n" +
				"AND QA_INSPECAO_SAIDA.EMPRESA_RECNO = CTE_SALDO_CQ.EMPRESA_RECNO\n" +
...
func (repo *OrdemProducaoRepository) BuscarOrdem(odf int, lote string) *models.OrdemProducao {
	var result models.OrdemProducao
...
func (repo *OrdemProducaoRepository) BuscarHisrealRelatorio(odf int, codigoProduto *string) *models.OrdemProducao {
	var result models.OrdemProducao

	query := queries.GetHisrealRelatorio
...
func (repo *OrdemProducaoRepository) BuscarClienteRelatorio(odf int) (*string, error) {
	var result *string

	query := queries.GetClienteRelatorio
...
func (repo *OrdemProducaoRepository) GetCustomAdvancedFilterFieldQuery(field string, value string) string {
	...
	if strings.ToUpper(field) == "ODFAPONTADA" {
		return "(PPEDLISE_ORDEM_FABRICACAO.NUMODF = '" + normalizedValue + "' OR CTE_SALDO_CQ.ODF = '" + normalizedValue + "')"
	}
