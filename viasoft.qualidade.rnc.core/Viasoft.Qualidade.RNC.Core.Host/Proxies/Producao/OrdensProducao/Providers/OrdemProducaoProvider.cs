...
public class OrdemProducaoProvider : IOrdemProducaoProvider, ITransientDependency
{
    ...
    public async Task<OrdemProducaoOutput> GetByNumeroOdf(int  numeroOdf, bool filtrarApenasEmitidas)
    {
        var environmentDetails = await _environmentStore.GetEnvironmentAsync(_currentEnvironment.Id.Value);

        var endpoint = $"{GetBaseEndpoint(environmentDetails.DesktopDatabaseVersion)}?Skip=0&PageSize=1&Filter={numeroOdf}";
        if (filtrarApenasEmitidas)
        {
            endpoint += "&Emitida=true";
        }
        else
