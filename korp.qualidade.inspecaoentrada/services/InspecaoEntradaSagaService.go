...
func (service *InspecaoEntradaSagaService) PublicarSagaInspecaoEntrada(input *dto.FinalizarInspecaoInput) (bool, *dto.ValidacaoDTO, error) {
	...
	if utilizarReservaDePedidoNaLocalizacaoDeEstoque {
		...
		for _, inspecaoPedido := range inspecoesEntrada {
			...
			transferencias, err := service.PreencherTransferenciasFromInspecaoEntrada(&inspecaoPedido, estoqueLocalValores, notaFiscal.CodigoLocal, descricaoOrigem, lotesAQuebrar)
			if err != nil {
				return false, nil, err
			}
		...
		movimentarEstoqueInput.Transferencias, err = service.PreencherTransferenciasFromInput(input, estoqueLocalValores, notaFiscal, descricaoOrigem)
		if err != nil {
			return false, nil, err
		}
...
func (service *InspecaoEntradaSagaService) PreencherTransferenciasFromInspecaoEntrada(
	...
	lotesAQuebrar []entities.InspecaoEntradaPedidoVendaLote) ([]dto.InspecaoEntradaTransferenciaBackgroundInputDto, error) {
...
func (service *InspecaoEntradaSagaService) PreencherTransferenciasFromInput(input *dto.FinalizarInspecaoInput, estoqueLocalValores *models.EstoqueLocalValores, notaFiscal *models.NotaFiscalModel, descricaoOrigem string) ([]dto.InspecaoEntradaTransferenciaBackgroundInputDto, error) {
	transferencias := make([]dto.InspecaoEntradaTransferenciaBackgroundInputDto, 0)
	numerosSeriesJaMovidos := make([]string, 0)
