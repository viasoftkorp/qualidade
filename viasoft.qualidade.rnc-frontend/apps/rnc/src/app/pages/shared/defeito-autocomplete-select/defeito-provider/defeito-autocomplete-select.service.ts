...
export class DefeitoAutocompleteSelectService {
  ...
  private proxyUrl = 'qualidade/rnc/gateway/defeitos';

  public getList(input:VsGridGetInput)
    : Observable<IPagedResultOutputDto<DefeitoOutput>> {
    ...
    return this.httpClient.get<IPagedResultOutputDto<DefeitoOutput>>(`${this.baseUrl}${this.proxyUrl}`, { params });
  }

  public get(id: string): Observable<DefeitoOutput> {
    return this.httpClient.get<DefeitoOutput>(`${this.baseUrl}${this.proxyUrl}/${id}`);
  }
}
