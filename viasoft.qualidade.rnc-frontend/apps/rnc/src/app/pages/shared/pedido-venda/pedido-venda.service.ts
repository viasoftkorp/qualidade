...
export class PedidoVendaService {
  ...
  constructor(@Inject(VS_BACKEND_URL) protected gateway: string, protected httpClient: HttpClient) {
    this.endpoint = `${ensureTrailingSlash(gateway)}qualidade/rnc/gateway/pedidos-venda`;
  }
  public getList(input: VsGridGetInput): Observable<IPagedResultOutputDto<PedidoVendaOutput>> {
    ...
    return this.httpClient.get<IPagedResultOutputDto<PedidoVendaOutput>>(this.endpoint, { params: httpParams });
  }
  public get(id: string): Observable<PedidoVendaOutput> {
    return this.httpClient.get<PedidoVendaOutput>(`${this.endpoint}/${id}`);
  }
}
