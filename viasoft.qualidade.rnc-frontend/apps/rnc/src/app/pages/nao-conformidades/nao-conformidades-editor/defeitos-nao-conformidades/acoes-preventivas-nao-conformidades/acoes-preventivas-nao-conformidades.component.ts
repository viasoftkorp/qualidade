...
import { AcoesPreventivasNaoConformidadesModel, DefeitosNaoConformidadesModel } from '@viasoft/rnc/api-clients/Nao-Conformidades';
 ...
 @Input() public defeito: DefeitosNaoConformidadesModel;
  ...
  private openEditorModal(action: EditorAction, data: AcoesPreventivasNaoConformidadesModel): void {
    ...
    this.matDialog.open(
      ...
      dialogConfig
    ).afterClosed().subscribe((acaoPreventiva: AcoesPreventivasNaoConformidadesModel) => {
      if (acaoPreventiva) {
        this.gridOptions.refresh();
      }
    });
