...
import { SolucaoModel, SolucaoInput } from '@viasoft/rnc/api-clients/Solucoes';
...
export class SolucaoService {
  ...
  private readonly endpointProdutoSolucao =
    (idSolucao: string) => `${ensureTrailingSlash(this.gateway)}qualidade/rnc/gateway/solucoes/${idSolucao}/produtos`;
  private readonly endpointServicoSolucao =
    (idSolucao: string) => `${ensureTrailingSlash(this.gateway)}qualidade/rnc/gateway/solucoes/${idSolucao}/servicos`;

  constructor(@Inject(VS_BACKEND_URL) protected gateway: string, protected httpClient: HttpClient) {
    this.endpointSolucao = `${ensureTrailingSlash(gateway)}qualidade/rnc/gateway/solucoes`;
  }
  ...
  public getViewList(input: VsGridGetInput): Observable<IPagedResultOutputDto<SolucaoModel>> {
    ...
    return this.httpClient.get<IPagedResultOutputDto<SolucaoModel>>(`${this.endpointSolucao}/view`, { params: httpParams });
  }
  ...
  public ativar(id: string): Observable<void> {
    return this.httpClient.patch<void>(`${this.endpointSolucao}/${id}/ativacao`, {});
  }
  public inativar(id: string): Observable<void> {
    return this.httpClient.patch<void>(`${this.endpointSolucao}/${id}/inativacao`, {});
  }
