...
public class NaoConformidadeViewHandlerNaoConformidadeInseridaMessageTests : NaoConformidadeViewHandlerTest
{
    [Fact(DisplayName = "Se não tiver id pessoa, nenhum registro de pessoa deve ser feito")]
    public async Task NaoConformidadeInseridaMessageTest1()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
        var message = new NaoConformidadeInserida
        {
    ...
    public async Task NaoConformidadeInseridaMessageTest2()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
    ...
    public async Task NaoConformidadeInseridaMessageTest3()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
    ...
    [Fact(DisplayName = "Se houver idCriador, deve chamar InserirUsuarioSeNaoCadastrado")]
    public async Task NaoConformidadeInseridaMessageTest5()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
        ...
        await handler.Handle(message);
        //Assert
        await mocker.FakeUsuarioService.Received(1).InserirSeNaoCadastrado(TestUtils.ObjectMother.Guids[0]);
    }
    
    [Fact(DisplayName = "Se houver nota fiscal e origem igual a inspeção de entrada, deve buscar número da nota fiscal de entrada ")]
    public async Task NaoConformidadeInseridaMessageTest6()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
        ...
        naoConformidadeAtualizada.Should().BeEquivalentTo(expectedResult, options => ExcludeAuditoria(options));
    ...
    public async Task NaoConformidadeInseridaMessageTest7()
    {
        //Arrange
        var mocker = GetMocker();
        var handler = GetHandler(mocker);
    ...
    private EquivalencyAssertionOptions<T> ExcludeAuditoria<T>(EquivalencyAssertionOptions<T> options) where T : FullAuditedEntity
    {
