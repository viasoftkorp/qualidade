...
public class DefeitoProvider : IDefeitoProvider, ITransientDependency
{
    private readonly IApiClientCallBuilder _apiClientCallBuilder;

    private const string ServiceName = "Viasoft.Qualidade.RNC.Core";
    private const string BasePath = "/qualidade/rnc/core/defeitos";

    public DefeitoProvider(IApiClientCallBuilder apiClientCallBuilder)
    {
    ...
    public async Task<DefeitoOutput> Get(Guid id)
    {
        ...
        var defeito = await callBuilder.ResponseCallAsync<DefeitoOutput>();
        return defeito;
    }
    ...
    public async Task<PagedResultDto<DefeitoOutput>> GetList(PagedFilteredAndSortedRequestInput input)
    {
        ...
        var defeito = await callBuilder.ResponseCallAsync<PagedResultDto<DefeitoOutput>>();
        return defeito;
    }
    ...
    public async Task<HttpResponseMessage> Create(DefeitoInput input)
    {
        var callBuilder = _apiClientCallBuilder
            .WithServiceName(ServiceName)
    ...
    public async Task<HttpResponseMessage> Update(Guid id, DefeitoInput input)
    {
        var callBuilder = _apiClientCallBuilder
            .WithServiceName(ServiceName)
            .WithEndpoint($"{BasePath}/{id}")
